...
- 人生清单：支持新增、编辑、删除 bucket list 项目，可设置目标日期，并可按自定义分类浏览。

需要登录后使用。登录后，数据会自动保存到 Supabase。使用同一个账号在手机和电脑登录，可以看到同一份清单。

## Supabase 初始化

在 Supabase 项目的 `SQL Editor` 里运行 `supabase-schema.sql` 一次，用来创建或更新数据表和账号级数据隔离策略。
